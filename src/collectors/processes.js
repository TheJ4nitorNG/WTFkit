...
function parseWmic(csvOutput) {
    ...
    // wmic output has headers on the first non-empty line (Node, CommandLine, Name, ProcessId)
    // We skip the first line (header)
    for (let i = 1; i < lines.length; i++) {
        const line = lines[i];
        const parts = line.split(',');
        if (parts.length >= 3) {
            // Because CommandLine might contain commas, we extract Name and ProcessId from the end
